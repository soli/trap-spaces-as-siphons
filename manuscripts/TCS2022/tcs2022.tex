...
Besides minimal trap spaces, these special types also play crucial roles in analyzing Boolean networks~\cite{Rozum2021}.
...
Finally, we conduct a more comprehensive benchmark on more real-world models from various sources and randomly generated models to evaluate all the proposed methods (the benchmark conducted in~\cite{DBLP:conf/cmsb/TrinhBHS22} considers only dozens of representative real-world models).
...
As such, we will explore how this transformation, here using BDDs in \texttt{bioLQM} and directly in our tool using the \texttt{pyeda}\footnote{\url{https://pyeda.readthedocs.io/en/latest/}} library, and the one based on the most-permissive semantics compare in the Section~\ref{sec:eval} on evaluation.
...
In the field of systems biology, biologists may want to compute more special types of trap spaces beyond minimal trap spaces~\cite{klarner2017pyboolnet}, which also play crucial roles in analysis of Boolean networks.
We shall show that our proposed methods can be easily adjusted to compute popular types of trap spaces.
We illustrate the adjustments via the ASP-based method (see Subsection~\ref{subsec:computation_asp}), but these adjustments are completely applicable for other approaches such as MaxSAT, CP, and ILP\@.
...
There are four variants of \texttt{Trappist}: \texttt{SAT} (i.e., the MaxSAT-based method shown in Subsection~\ref{subsec:computation_csp}), \texttt{CP} (i.e., the CP-based method shown in Subsection~\ref{subsec:computation_csp}), \texttt{ILP} (i.e., the ILP-based method shown in Subsection~\ref{subsec:computation_ilp}), and \texttt{ASP} (i.e., the ASP-based method shown in Subsection~\ref{subsec:computation_asp}).
...
For 3 of the 29 models, \texttt{mpbn} did not give any answer because these models are locally-monotonic.
...
%		]
%		coordinates {
%			(0.5, 143)(1, 176)(2, 195)(5, 222)(10, 249)(20, 264)(50, 287)(100, 308)(180, 322)
...
%		]
%		coordinates {
%			(0.5, 144)(1, 177)(2, 196)(5, 223)(10, 250)(20, 265)(50, 288)(100, 309)(180, 323)
%		}; \addlegendentry{\texttt{ILP}}
	...
	\end{semilogxaxis}
	\end{tikzpicture}
	\caption{Cumulative numbers of the \texttt{BBM} models that have less than 1000 minimal trap spaces (above) and \texttt{BBM} models (below) solved by the competing methods with respect to enumerating the first 1000 minimal trap spaces.}
	\label{fig:BBM_cumul}
\end{figure}

Figure~\ref{fig:BBM_cumul} (above) shows cumulative numbers of the \texttt{BBM} models that have less than 1000 minimal trap spaces solved by the competing methods with respect to enumerating the first 1000 minimal trap spaces.
The number of such models is 134 (per all 211 models), and 15 of them are non-locally-monotonic.
We can first see that \dots


Figure~\ref{fig:BBM_cumul} (below) shows cumulative numbers of the \texttt{BBM} models solved by the competing methods (except \texttt{bioLQM}) with respect to enumerating the first 1000 minimal trap spaces. \dots
...
The first observation is that for 11 of the 32 models (more than 34\%), \texttt{mpbn} did not give any answer because these models as not locally-monotonic.
For 22 of the 32 models where \texttt{mpbn} returned the answers, \texttt{mpbn} and \texttt{Trappist} are comparable in computation time, though surprisingly \texttt{mpbn} appears quite slower on average than the best variant of \texttt{Trappist}.
In particular, for the Rho-GTPases model, \texttt{mpbn} is \(577\times\) slower than the best variant of \texttt{Trappist}.
...
However, since \texttt{mbpn} can handle only locally-monotonic models and \texttt{Trappist} can handle general models, it is difficult to further compare between them.
Hence, we focus on only comparisons among \texttt{bioLQM}, \texttt{PyBoolNet}, and \texttt{Trappist} in the following observations.
...
\begin{figure}[ht]
	...
	\end{semilogxaxis}
	\end{tikzpicture}
	\caption{Cumulative numbers of random models solved by the competing methods with respect to enumerating all the minimal trap spaces.}
...
Figure~\ref{fig:random_cumul_first} shows cumulative numbers of random models solved by the competing methods with respect to enumerating all the minimal trap spaces.
The number of succeeded models within three minutes for each method is as follows: \texttt{bioLQM} (0), \texttt{PyBoolNet} (320), \texttt{mpbn} (0), \texttt{Trappist-maxSAT} (338), \texttt{Trappist-CP} (0), \texttt{Trappist-ILP} (0), \texttt{Trappist-ASP} (349).
We can see that \texttt{Trappist-ASP} is the method that could handle the most number of models.
Note that none of the other methods could handle the model failed by \texttt{Trappist-ASP}.
Upon closer inspection, we obtained several observations consistent with those obtained for real-world models as follows.
...
Hence, it is apparent that all the randomly generated models are non-locally-monotonic.
This observation confirms the limit on the applicable model class of \texttt{mpbn}.

Second, surprisingly \texttt{bioLQM} could not handle any model.
...
It is apparent because the network size is large (\(\geq 100\)) and the Boolean functions are not simple.

Third, \texttt{PyBoolNet} could handle every model of network size less than or equal to 300.
It only failed in one model of network size 350 but 29 models of network size 400.
The average running time vastly increases as the network size increases.
As compared to the four methods of our approach, the performance of \texttt{PyBoolNet} is comparable for the 100-node and 200-node models.
However, from \(n = 250\), the performance difference is exhibited more clearly.  
...
The experimental results show that the new approach vastly outperforms all the state-of-the-art methods in terms of general Boolean networks and is comparable to the \texttt{mpbn} method even better in average in terms of locally-monotonic Boolean networks.
...
We also believe that the connection between trap spaces of Boolean networks and siphons of Petri nets can be a very useful tool for exploring and proving more new properties of trap spaces in Boolean networks, as we have used it to successfully prove the separation of minimal trap spaces.
