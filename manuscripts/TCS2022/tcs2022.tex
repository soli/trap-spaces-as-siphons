...
Note that the above computation (also the Petri net construction) is quadratic in the number of transitions of the Petri net in worst cases, which can be much smaller than the number of prime-implicants~\cite{chatain2014characterization}.
...
To evaluate the performance of the newly proposed methods (implemented as a Python package named Trappist) and the state-of-the-art methods (bioLQM\footnote{\url{http://colomoto.org/biolqm/doc/tools-trapspace.html}}, PyBoolNet~\cite{klarner2015computing,klarner2017pyboolnet}, and mpbn~\cite{Paulev2020}), we compared them on both PyBoolNet's own model repository and many real-world models from various sources in the literature.
It is worth noting that mpbn~\cite{Paulev2020} only handles locally-monotonic models, whereas the other methods can handle general models.
...
As explained in Section~\ref{sec:case_study}, in our benchmarks, we only searched for the first 1000 solutions for each model.

To solve the ASP problems, we used the same ASP solver CLINGO~\cite{DBLP:journals/aicom/GebserKKOSS11} and the same configuration as that used in PyBoolNet~\cite{klarner2015computing,klarner2017pyboolnet} and mpbn~\cite{Paulev2020}.
Specifically, we used the configuration \texttt{-heuristic=Domain -enum-mod=domRec -dom-mod=3} (subset maximality, equivalent to the deprecated \texttt{--dom-pref=32 --heuristic=domain --dom-mod=7} used by PyBoolNet). 
...
\begin{table}[!htb]
  \caption{Timing comparisons (in seconds) between bioLQM (LQM), PyBoolNet (PBN), mpbn and the four variants of Trappist on the PyBoolNet repository.}
  \centering%
...
Table~\ref{tab:pyboolnet_repo} shows the experimental results on the models from the official PyBoolNet repository\footnote{\url{https://github.com/hklarner/pyboolnet/tree/master/pyboolnet/repository}}.
...
As shown in Table~\ref{tab:pyboolnet_repo}, for most of the models of the PyBoolNet repository, the results are comparable with all minimal trap spaces found very fast.
...
For the case of bioLQM, we only considered the models that have at most 1000 minimal trap spaces.
The number of such models is 134 (per all 211 models) and is denoted inside the parentheses.
For the case of mpbn, we only considered the models that are locally-monotonic.
...
Note that these models are not included in the PyBoolNet and BBM repositories.
We then applied bioLQM, PyBoolNet, mpbn, and the four variants of Trappist to computing minimal trap spaces of these real-world models.
...
In the case of bioLQM, ``N/A'' means that the number of all minimal trap spaces of the model is larger than 1000 and we did not recorded the running time of bioLQM because it always requires to compute all minimal trap spaces.
...
\begin{table}[!htb]
  \caption{Timing comparisons (in seconds) between bioLQM (LQM), PyBoolNet (PBN), mpbn and the four variants of Trappist on selected models from the literature.}
...
Hence, PyBoolNet always passed the phase of computing prime-implicants in every model even within 1s, which enables us to compare the ASP encoding of PyBoolNet and that of Trappist.
...
\begin{table}[!htb]
  ...
  \begin{tabular}{rrrrrrrr}
    ...
    & & & & \multicolumn{4}{c}{Trappist}\\
    \cmidrule(rr){5-8}
    \(n\) & LQM & mpbn & PBN & SAT & CLP & ILP & ASP \\
    \midrule
