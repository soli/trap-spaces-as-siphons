...
There are many different update schemes, but the two main types~\cite{thomas1991regulatory} are: synchronous, where all the nodes are update simultaneously, and fully asynchronous, where only one node is selected in random to be updated.
...
\sylvain{I'm not sure how much we should insist on the ``surprise''. The proof directly on trap-spaces is quite obvious and follows the same structure: the intersection would be a trap-space, and smaller than the original ones, hence a contradiction\ldots}
\vangiang{I agree with you. I have added an argument for this.}
...
Note that there are no existing methods specifically designed for computing maximal conflict-free siphons (even maximal siphons) of a Petri net.
...
For the ``RA-apoptosis'' model, using the ASP-based method presented in Section~\ref{subsec:computation_asp}, it is possible to obtain the first 1000 minimal trap spaces (including ones that contain more than one state) within 0.19s, which is much quicker than \texttt{mpbn}.
...
\sylvain{I think that the result above (0.19s) is only about the F5 model, but do we also have the time for the F4 model, if so, it should be nice to include it too.}
\vangiang{I agree. Simply included it.}
...
\sylvain{should we also highlight in some way the (tied-)best result?}
\vangiang{I think we should not because such highlights seem not much support the experimental analysis, but may make the table a bit awkward.}
...
However, the reason may lie on the differences in the ASP encoding characteristics of the two methods and the fact that \texttt{mpbn} needs to spend time checking the locally-monotonicity of each Boolean function in a Boolean network.
...
In particular, we have shown how to adjust our approach to compute several specific types of trap spaces (e.g., maximal trap spaces, fixed points), which besides minimal trap spaces also play crucial rules in the analysis and control of Boolean networks.
...
\sylvain{Do you want to say a word about specific types of trap-spaces? This currently does not really appear in the conclusion.}
\vangiang{Thank you for your reminder. I have mentioned the computation of specific types of trap spaces in the conclusion.}
