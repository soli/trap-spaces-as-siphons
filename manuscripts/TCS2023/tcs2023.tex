...
\begin{subfigure}[b]{0.25\textwidth}
  ...
  \begin{tikzpicture}
    ...
    \draw[->] (2) edge [bend left=45] (1);
    \draw[->] (1) to [loop left] (1);
    \draw[->] (2) to [loop right] (2);
  \end{tikzpicture}
  \caption{\change{Influence graph of the model. Each arc is actually both positive and negative.}}
\end{subfigure}\hfill%
\begin{subfigure}[b]{0.25\textwidth}
\centering
...
\label{fig:PN}
\end{subfigure}\hfill%
\caption{\change{Influence graph, dynamics, and Petri net encoding of the Boolean network of Example~\ref{example:BN}}.}%
...
\change{The research group behind the \texttt{BBM} repository~\cite{pastva2023repository} has recently undertaken considerable effort for building a collection of real-world Boolean models from various sources used in systems biology.}
...
Second, although these reduction techniques can reduce the number of nodes, they can also increase the complexity of Boolean update functions~\cite{DBLP:journals/tcs/NaldiRTC11}, which is also an important factor for the performance of computation methods.
