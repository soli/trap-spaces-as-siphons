...
\[\verb|:- p(v, 1), p(v, 0).|\]
for each \(v \in V\). This rule guarantees that \verb|p(v, 1)| and \verb|p(v, 0)| never belong to the same answer set of \(\mathcal{A}\), representing the conflict-freeness. Note that the number of atoms of \(\mathcal{A}\) is only \(2n\), which is more theoretically advanced than the ASP encoding shown in~\cite{klarner2015computing} where the number of atoms of the encoded ASP equals to the number of prime-implicants of the Boolean model and that number might be exponential in \(n\).

% \sylvain{What do you mean by \emph{advanced}? It is less since there might be an exponential number of prime-implicants?}
% \vangiang{Yes. Since there might be an exponential number of prime-implicants, our new ASP encoding is more advanced than the ASP encoding of PyBoolNet.}
...
This method is evaluated on big models from the literature and shows that it can scale up much better than the state-of-the-art prime-implicants based techniques.
