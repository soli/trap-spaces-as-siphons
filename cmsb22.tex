...
%%% Trap set %%%
Given a Boolean model \(\mathcal{M} = (V, F)\) with variables \(V=(v_{1},\dots,v_{n})\) and Boolean functions \(F=(f_{1},\dots,f_{n})\), its state-space is \(\mathcal{S}_{\mathcal{M}} =\bbbb^{n}\) with \(\bbbb = \{0, 1\}\). A non-empty set \(T \subseteq S_{\mathcal{M}}\) is a \emph{trap set} of \(\mathcal{M}\) if and only if for every \(x \in T\) and \(y \in S_{\mathcal{M}}\) with \(y\) is reachable from \(x\) it holds that \(y \in T\).
...
We shall write \(pred(x)\) (resp.\ \(succ(x)\)) to represent the set of vertices that have an (non-zero weighted) arc leading to (resp.\ coming from) \(x\).
...
Its basis is that every node (\emph{gene}) \(v\) of the original model is represented by two separate places (\(p_v\) and \(\overline{p}_v\)), corresponding to its two states, active, and inactive, respectively.
Each conjunct of the logical function that activate the \emph{gene} will lead to a transition \(t\), consuming the inactive place (i.e., a directional arc from \(\overline{p}_v\) to \(t\)), producing the active place (i.e., a directional arc from \(t\) to \(p_v\)), and with all other literals both consumed and produced (i.e., a bi-directional arc).
And conversely for the inactivation. 
Let \(s\) be a state of the Boolean model and \(m_s\) be its the corresponding marking in the encoded Petri net. It holds that \(\forall v \in V\), \(s(v) = 0\) if and only if \(m_s(\overline{p}_v) = 1\) and \(s(v) = 1\) if and only if \(m_s(p_v) = 1\).
...
\begin{definition}
  ...
  \[\forall v \in D_m, m(v) = 0 \Leftrightarrow p_v \in S, m(v) = 1 \Leftrightarrow \overline{p}_v \in S\] and \[\forall v \not \in D_m, p_v \not \in S, \overline{p}_v \not \in S.\] If \(D_m = \emptyset\), \(S\) is conventionally \(\emptyset\).
...
First, we show the characterization of a conflict-free siphon of the encoded Petri net \(\mathcal{P}\). ...

Then, we translate the above characterization into the ASP \(\mathcal{A}\) as follows. We use atom \texttt{p(v, 1)} (resp. \texttt{p(v, 0)}) to denote place \(p_v\) (resp. \(\overline{p_v}\)), \(\forall v \in V\). The constraint (C2) is translated to the rule
\[\verb|:- p(v, 1), p(v, 0).|\]
for each \(v \in V\). ...

Naturally, a stable model \(A\) of \(\mathcal{A}\) is equivalent to a conflict-free siphon \(S\) of \(\mathcal{P}\), thus a minimal trap space \(m\) of \(\mathcal{M}\). The conversion from \(A\) to \(m\) is straightforward. If \(\texttt{p(v, 1)} \in A\) then \(v \in D_m\) and \(m(v) = 0\). Conversely, if \(\texttt{p(v, 0)} \in A\) then \(v \in D_m\) and \(m(v) = 1\). Otherwise, \(v \not \in D_m\). Computing multiple solutions is built into ASP solvers and the solving collection POTASSCO~\cite{DBLP:journals/aicom/GebserKKOSS11} also features the option to find set-inclusion maximal solutions with respect to the predicates that are true. By using this built-in option, we can compute all the stable models of \(\mathcal{A}\) in one execution.
