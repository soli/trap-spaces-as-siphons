...
\revise{It is worth noting that there has been a recent method~\cite{DBLP:conf/ictai/ChevalierFPZ19} for computing minimal trap spaces, which avoids the prime-implicants computation.
...
\revise{In~\cite{DBLP:conf/ictai/ChevalierFPZ19}, there is an ASP characterization of trap spaces that does not rely on minimal DNFs and thus seems very similar to the our ASP encoding.
However, this encoding is only applicable for locally-monotonic Boolean models.
Hence it might be theoretically insightful to compare in terms of complexity between it and our ASP encoding only for locally-monotonic BNs, but this is out of the scope of this article.
}
...
\revise{Note that mpbn~\cite{Paulev2020} does not give any answer because it recognized that model non-locally-monotonic.}
...
In addition, we also include the tool mpbn~\cite{Paulev2020}\footnote{We use the latest version of mpbn available at submission time through CoLoMoTo.} to the benchmarks although it only handle locally-monotonic models, whereas both Trappist and PyBoolNet can handle general models.}
...
\revise{For 5 of the 29 models, mpbn did not give any answer because it recognized that these models are not locally-monotonic.
...
\revise{The first observation is that for 26 of the 33 models (more than 78\%), mpbn did not give any answer because it recognized that these models are not locally-monotonic.
For 6 of the 33 models where mpbn returned the answers, mpbn and Trappist are comparable in computation time.
Along with the fact that mbpn can handle only locally-monotonic models and Trappist can handle general models, it is less meaningful to further compare between them.
