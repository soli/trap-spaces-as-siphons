...
\begin{abstract}
  ...
  Boolean modelling of gene regulation but also of post-transcri\-ptomic systems has proven over the years that it can bring powerful analyses and corresponding insight to the many cases where precise biological data is not sufficiently available to build a detailed quantitative model.
...
\caption{}
\end{subfigure}%
\caption{(a) Dynamics under the fully asynchronous update of the Boolean model. (b) Petri net encoding of the Boolean model. Circles denote places, whereas rectangles denote transitions.}
...
To assess the efficiency of the proposed method, we compared it with the state-of-the-art method PyBoolNet~\cite{klarner2015computing,klarner2017pyboolnet} on both its own repository of models and large models from the literature.
...
\begin{table}[!htb]
  \caption{Timing comparisons between the proposed method (i.e., Trappist) and PyBoolNet on the PyBoolNet's repository.
    Column \(n\) denotes the number of nodes of each model, Column \(|M|\) denotes the number of minimal trap spaces and for each method is given the computation time in seconds.
...
On the contrary, on every model that was a bit challenging for PyBoolNet, the new method was far more efficient with speedups between one and two orders of magnitude.
...
The first observation is that the proposed method much outperforms PyBoolNet in computational time.
More specifically, ...
...
The third observation is that for very huge models (), the proposed method did not manage to finish the Petri net conversion before the timeout, as marked by ``???'' in Table~\ref{tab:result_real}.
...
Second, with the experimental results shown in this paper, it is not enough information to distinctly expose the practical differences between our ASP encoding and that of PyBoolNet.
