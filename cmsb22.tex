...
To stay in line with the usual considerations we will always assume purely Boolean (and not multi-valued) logical models, and an asynchronous update.
In the case of multi-level models, an encoding into a Boolean model is always possible (and once again available through BioLQM).
As far as different update strategies are concerned, we think that even if they might require small technical adaptations, they would not bring much change to the main results of this article.
\vangiang{Minimal trap spaces are a static property of a BN regardless of its update semantics. All the results presented in this paper still valid for any update semantics. Hence, we do not need to restrict to an asynchronous update.}
...
Note that transforming a Boolean model defined by its functions into its Petri net encoding roughly relies on obtaining the Disjunctive Normal Forms (DNF) of each Boolean function.\vangiang{I think it is better to put a reference here for this argument.}
Though this might be quite computationally intensive it is important to remark first that contrary to the Prime implicants case, there is no need to find \emph{minimal} DNF.
Second, in practice this transformation seems to behave quite well on even quite big models as will be shown in the Section~\ref{sec:eval} on evaluation.
Currently, relying on BioLQM implies going through a BDD encoding of the formulae, which might not be optimal.
Hence we are still considering a direct encoding that would skip the BDD construction.
...
First, we show the characterization of all conflict-free siphons of the encoded Petri net \(\mathcal{P} = (P, T, W)\). Suppose that \(S\) is a generic siphon of \(\mathcal{P}\). If a place \(p\) should belong to \(S\), then by definition all the transitions in \(pred(p)\) must belong to \(succ(S)\). Note that \(succ(S) = \bigcup_{p \in S}succ(p)\). A transition \(t\) belongs to \(post(S)\) if and only if there is at least one place \(p'\) in \(S\) such that \(p' \in pred(t)\). Hence, for each transition \(t \in pred(p)\), we can state that 
\begin{equation}
\label{eq:siphon}
...
\] where \verb|p(v, x)| is the atom representing place \(p\) and \(\{\verb|p(v_1, x_1)|, ..., \verb|p(v_k, x_k)|\}\) is the set of atoms representing places in \(pred(t)\). The rule (\ref{eq:conflict}) is translated into the ASP rule
\[\verb|:- p(v, 1), p(v, 0).|\]
for each \(v \in V\). This rule guarantees that \verb|p(v, 1)| and \verb|p(v, 0)| never belong to the same answer set of \(\mathcal{A}\), representing the conflict-freeness. Note that the number of atoms of \(\mathcal{A}\) is only \(2n\), which is theoretically advanced to the ASP encoding shown in~\cite{klarner2015computing} where the number of atoms of the encoded ASP equals to the number of prime-implicants of the Boolean model.
...
In the supplementary material of the paper, an excerpt of the map, focused around the apoptosis (cell death) module is transformed into a model of \emph{reasonable} size, namely 181 Boolean variables (model \verb|F5_RA_apoptosis_executable_module.sbml|).
...
In practice, even if there are many attractors in such a model, to validate it having only a sample of those was deemed useful.
...
Using the method presented above, it is possible to convert the model to PNML in about 1s and to obtain the first 1000 minimal trap spaces (including ones that contain more than one state) in a few milliseconds.
