...
To stay in line with the usual considerations we will always assume purely Boolean logical models, and an asynchronous update.
In the case of multi-level models, an encoding into a Boolean model is always possible.
...
Note that transforming a Boolean model defined by its functions into its Petri net encoding roughly relies on obtaining the Disjunctive Normal Forms (DNF) of each Boolean function.
Though this might be quite computationally intensive it is important to remark first that contrary to the prime-implicants case, there is no need to find \emph{minimal} DNFs.
...
\sylvain{What do you mean by \emph{advanced}? It is less since there might be an exponential number of prime implicants?}
...
In this article we proposed a new method for the computation of minimal trap-spaces of Boolean models, based on maximal siphons.
This method is evaluated on big models from the litterature and shows that it can scale up much better than the state-of-the-art prime-implicants based techniques.
We believe that this opens up the way to a much better analysis of large Boolean models, which is needed with the advent of automatic model-generation pipelines~\cite{ONMKPO21molecular}.
